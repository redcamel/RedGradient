...
class SplitterLayout extends React.Component {
	...
	getSecondaryPaneSize(containerRect, splitterRect, clientPosition, offsetMouse) {
		...
		if (this.props.primaryMaxSize) {
			if (primaryPaneSize > this.props.primaryMaxSize) {
				return window.innerWidth - this.props.primaryMaxSize
			}
		}
	...
	handleResize() {
		if (this.splitter && !this.props.percentage) {
			...
			const secondaryPaneSize = this.getSecondaryPaneSize(containerRect, splitterRect, {
				left: this.props.primaryMaxSize ?
					(window.innerWidth - +splitterRect.left > this.props.primaryMaxSize ? this.props.primaryMaxSize : splitterRect.left) : splitterRect.left,
	...
	render() {
		...
		return (
			<div className={containerClasses} ref={(c) => {
				this.container = c;
			}}>
				{wrappedChildren[0]}
