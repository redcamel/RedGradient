...
import drawCanvasEditUI from "./edit/drawCanvasEditUI.js";
// TODO - 정리필요
...
class RedCanvas extends React.Component {
  ...
  drawCall(targetState, activeYn, index) {
    const activeSubData = targetState.activeSubData;
    const canvasInfo = targetState['canvasInfo']
    const layers = targetState['layers']
    const activeLayer = targetState['activeLayer']
    ...
    const borderGradientInfo = targetState.borderGradientInfo;
    ...
    if (activeYn && ghostMode && !ghostSize) ghostSize = {...layoutSize};
    const iconScale = Math.min(1, 1 / this.state.canvasViewScale)
    return <div
      style={{
        ...style.canvas,
        border: activeYn ? '1px solid rgba(255,255,255,0.0)' : '1px solid rgba(255,255,255,0.1)',
        transform:
          `translate(calc(-50% + ${this.state.canvasViewOffsetX}px),calc(-50% + ${this.state.canvasViewOffsetY}px)) scale(${this.state.canvasViewScale})`
      }} className={index ? '' : 'transparent_checker'}>
      <div
