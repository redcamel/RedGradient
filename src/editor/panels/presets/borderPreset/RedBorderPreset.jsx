import RedBorderPresetItem from "./RedBorderPresetItem";
...
const RedBorderPreset = () => {
	...
	const [userPresetList, setUserBorderPresetList] = useState(makePresetDataList([]))
	useEffect(() => {
		const HD_addUserBorderPreset = (e) => {
			const data = e?.detail
			const localList = JSON.parse(localStorage.getItem('userBorderGradientDataList') || "[]")
			if (data) {
				localList.push(JSON.parse(JSON.stringify(data)))
				localStorage.setItem('userBorderGradientDataList', JSON.stringify(localList))
			}
			const newUserBorderPresetList = makePresetDataList(localList);
			setUserBorderPresetList(newUserBorderPresetList)
		}
		const HD_removeUserBorderPreset = (e) => {
		...
		HD_addUserBorderPreset()
		window.addEventListener('addUserBorderPreset', HD_addUserBorderPreset)
		window.addEventListener('removeUserBorderPreset', HD_removeUserBorderPreset)
		return () => {
			window.removeEventListener('addUserBorderPreset', HD_addUserBorderPreset)
			window.removeEventListener('removeUserBorderPreset', HD_removeUserBorderPreset)
		}
	}, [])
	const renderMenu = () => {
		const menuList = [
						...
						onClick={() => {
							switch (index) {
								...
								case 2 :
									...
									a.download = `RedGradientUserBorderPreset.preset.json`;
									a.click();
									...
									a.onchange = e => {
										...
										fileReader.onload = evt => {
											...
											if (checkValidate(newData)) {
												localStorage.setItem('userBorderGradientDataList', newData);
												const newUserBorderPresetList = makePresetDataList(JSON.parse(newData));
												setUserBorderPresetList(newUserBorderPresetList)
	...
	return <div style={{
		...
		<div style={{
			...
			<div style={{padding: '0 10px'}}>Border Gradient Preset</div>
			<div style={{
				overflow: 'hidden',
				borderRadius: '4px',
				display: 'flex',
		...
		<div style={stylePresetContainer}>
			...
			{/*			style={styleListBox}>*/}
			{/*			{*/}
			{/*				systemPresetList[activeIdx]['list'].map((v, index) => <RedBorderPresetItem data={v} key={index}/>)*/}
			{/*			}*/}
			{/*		</div>*/}
			{/*	}*/}
			{/*	</div>*/}
					...
					<div
						style={styleListBox}>
						{
							userPresetList[activeIdx]['list'].length
								? userPresetList[activeIdx]['list'].map((v, index) => <RedBorderPresetItem
									key={index}
									data={v}
...
export default RedBorderPreset
